...
class B_PATCHGAN_3D_BINARY(object):
    ...
    def __init__(self, x_dim, batch_size=64, gf_dim=64, df_dim=64,
                 ...
                 ml=False, num_train=64, l1lambda=0.1, patch_num=3, channel=1):  # eta=2e-4,
    ...
    def construct_from_hypers(self, gen_kernel_size=3, num_dfs=None, num_gfs=None):
        ...
        self.sup_g_batch_norm = AttributeDict([("sup_g_bn%i" % gbn_i, batch_norm(name='sup_g_bn%i' % gbn_i)) for gbn_i in range(self.num_gen_layers)])
...
#        self.gen_output_dims = OrderedDict()
        ...
        self.gen_weight_dims["g_h%i_W" % 0] = (3, 3, 3, 1, num_gfs[0])  # from the image
        ...
        self.gen_weight_dims["g_h%i_W" % 9] = (3, 3, 3, num_gfs[9], num_gfs[9])  # conv5
...
#########################################################################################################
        ...
        self.disc_weight_dims["d_h%i_W" % 0] = (5, 5, 5, 1, num_dfs[0])  # output = ( s_h / 2, s_w / 2 )
...
#####################################################################################################################
        ...
        for zi in range(self.num_gen):
            self.distrib_weight_dims["dist_%i_mu" % zi] = (1, self.x_dim[0], self.x_dim[1], self.x_dim[2])   # self.num_classes
            self.distrib_weight_dims["dist_%i_var" % zi] = (1, self.x_dim[0], self.x_dim[1], self.x_dim[2])
...
#####################################################################################################################
    ...
    def build_bpgan_graph(self):
        ...
        data_pair = tf.placeholder(tf.float32, [self.batch_size * (1 + self.num_patches), self.x_dim[0], self.x_dim[1], self.x_dim[2], 2], name='stacked_data_pair')

        self.imgs = data_pair[:, :, :, :, 1: 2]  #  self.imgs = data_pair[:, :, :, :, self.channel: self.channel + self.channel]
        self.labels = data_pair[:, :, :, :, :1]  #  self.labels = data_pair[:, :, :, :, : self.channel]

        # self.labels = tf.placeholder(tf.float32, [self.batch_size * (1 + self.num_patches), self.x_dim[0], self.x_dim[1], self.x_dim[2]], name='input_labels')
        # self.one_hot_labels = tf.one_hot(tf.to_int32(self.labels), depth=self.num_classes)  # reduce the last dim

        self.pos_pair = tf.concat([self.imgs, self.labels], axis=4)
...
#        tf.summary.histogram("d_real", d_loss_real)
        ...
        for gi, gen_params in enumerate(self.gen_param_list):
            ...
            predict_sigmoid = tf.nn.sigmoid(logit)
            predicts_bp.append(predict_sigmoid)
            self.predicts.append(predict_relu)   # (batch, h, w, d, 1)

            ##################################

            fake_AB = tf.concat([self.imgs, predict_sigmoid], axis=4)   # predict

            seg_loss_dice = 1. - dice_coef(predict_relu, self.labels)   # self.labels
            seg_loss_cross = tf.losses.sigmoid_cross_entropy(logits=predict_sigmoid, multi_class_labels=self.labels)
...
#            tf.summary.histogram("d_loss_fake", self.d_losses)
    ...
    ####################################################################################################################
            ...
            self.gen_fused, exp_vars, log_vars = self.distrib_classifier(predicts_bp, self.distrib_param_list[0])   # self.predicts

            fused_loss_seg = 1.0 - dice_coef(self.labels, self.gen_fused) + \
                             tf.losses.sigmoid_cross_entropy(logits=self.gen_fused, multi_class_labels=self.labels)

            fused_loss = tf.multiply(exp_vars, fused_loss_seg) + log_vars

            self.distrib_losses.append(1.0 - dice_coef(self.labels, self.gen_fused))   # this loss is to print
    ...
    def generator(self, image, gen_params):
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE) as scope:
            ...
            pool4 = maxpooling_3d(conv4, name='maxpool4')
            pool4 = tf.nn.dropout(pool4, .5)


            conv5 = conv3d(pool4, 16 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv5_conv1', w=gen_params["g_h%i_W" % 8], biases=gen_params["g_h%i_b" % 8])
            ...
            logit = conv3d(conv9, 1, k_h=1, k_w=1, k_d=1, d_h=1, d_w=1, d_d=1, name='conv_logit', w=gen_params["g_h%i_W" % 18], biases=gen_params["g_h%i_b" % 18])
