...
class BDCGAN(object):
    ...
    def __init__(self, x_dim, z_dim, dataset_size, batch_size=64, gf_dim=64, df_dim=64, 
        ...
        self.gf_dim = gf_dim
        self.df_dim = df_dim
        self.c_dim = c_dim  # channel
        self.lr = lr
        
        # Bayes
        self.prior_std = prior_std   # variance sigma = 1.0
        self.num_gen = J
    ...
    def initialize_wgts(self, scope_str):
        ...
        with tf.variable_scope(scope_str) as scope:
            for zi in range(numz):                # num_disc : J_d
                for m in range(self.num_mcmc):    # number of MCMC neural network weights per sample
                    wgts_ = AttributeDict()
    ...
    def build_bgan_graph(self):
        ...
        self.gen_param_list = self.initialize_wgts("generator")          # J / num_gen * num_mcmc
        self.disc_param_list = self.initialize_wgts("discriminator")     # J_d / num_dics * num_mcmc
        ...
        for di, disc_params in enumerate(self.disc_param_list):
            ...
            for d_loss_fake_ in d_loss_fakes:
                ...
                if not self.ml:
                    ...
                    print("disc_params: ", len(disc_params), self.disc_prior(disc_params), self.disc_noise(disc_params))
