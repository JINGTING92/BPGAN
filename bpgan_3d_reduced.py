...
class B_PATCHGAN_3D_REDUCE(object):
    ...
    def __init__(self, x_dim, batch_size=64, gf_dim=64, df_dim=64,
                 ...
                 ml=False, num_train=64, l1lambda=0.1, channel=1, more_pooling=False, less_pooling=False):  # eta=2e-4,
        ...
        self.more_pooling = more_pooling
        self.less_pooling = less_pooling
    ...
    def construct_from_hypers(self, gen_kernel_size=3, num_dfs=None, num_gfs=None):
        ...
        if num_gfs is None:
            ...
            num_gfs = [self.gf_dim, self.gf_dim,
                       self.gf_dim * 2, self.gf_dim * 2,
                       self.gf_dim * 4, self.gf_dim * 4,
                       self.gf_dim * 8, self.gf_dim * 8,   # middle layer
...
#        self.gen_output_dims = OrderedDict()
        ...
        self.gen_weight_dims["g_h%i_W" % 8] = (3, 3, 3, num_gfs[5] + num_gfs[7], num_gfs[8])     # conv6 concat conv4
        ...
        self.gen_weight_dims["g_h%i_W" % 10] = (3, 3, 3, num_gfs[9] + num_gfs[3], num_gfs[10])  # conv7 concat conv3
        ...
        self.gen_weight_dims["g_h%i_W" % 12] = (3, 3, 3, num_gfs[11] + num_gfs[1], num_gfs[12])  # conv8 concat conv2
        ...
        self.gen_weight_dims["g_h%i_W" % 14] = (1, 1, 1, num_gfs[13], num_gfs[14])
...
#        tf.summary.histogram("d_real", d_loss_real)
        ...
        for gi, gen_params in enumerate(self.gen_param_list):
            ...
            if self.num_classes == 2:
                predict_softmax = tf.nn.sigmoid(logit)
            else:
...
#            tf.summary.histogram("d_loss_fake", self.d_losses)
    ...
    def generator(self, image, gen_params):
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE) as scope:
            ...
            conv5 = conv3d(pool3, 8 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv5_conv1', w=gen_params["g_h%i_W" % 6], biases=gen_params["g_h%i_b" % 6])
            conv5 = self.g_batch_norm["g_bn%i" % 6](conv5)
            conv5 = tf.nn.relu(conv5, name='conv5_relu1')
            conv5 = conv3d(conv5, 8 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv5_conv2', w=gen_params["g_h%i_W" % 7], biases=gen_params["g_h%i_b" % 7])
            conv5 = self.g_batch_norm["g_bn%i" % 7](conv5)
            ...
            up2 = unpool3d(conv5)  # up2 = tf_utils.upsampling3d(conv6, size=(2, 2, 2), name='conv7_up')
            conv7 = tf.concat([up2, conv3], axis=4, name='conv7_concat')
            conv7 = conv3d(conv7, 4 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv7_conv1', w=gen_params["g_h%i_W" % 8], biases=gen_params["g_h%i_b" % 8])
            conv7 = self.g_batch_norm["g_bn%i" % 8](conv7)
            conv7 = tf.nn.relu(conv7, name='conv7_relu1')
            conv7 = conv3d(conv7, 4 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv7_conv2', w=gen_params["g_h%i_W" % 9], biases=gen_params["g_h%i_b" % 9])
            conv7 = self.g_batch_norm["g_bn%i" % 9](conv7)
            ...
            conv8 = conv3d(conv8, 2 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv8_conv1', w=gen_params["g_h%i_W" % 10], biases=gen_params["g_h%i_b" % 10])
            conv8 = self.g_batch_norm["g_bn%i" % 10](conv8)
            conv8 = tf.nn.relu(conv8, name='conv8_relu1')
            conv8 = conv3d(conv8, 2 * self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv8_conv2', w=gen_params["g_h%i_W" % 11], biases=gen_params["g_h%i_b" % 11])
            conv8 = self.g_batch_norm["g_bn%i" % 11](conv8)
            ...
            conv9 = conv3d(conv9, self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv9_conv1', w=gen_params["g_h%i_W" % 12], biases=gen_params["g_h%i_b" % 12])
            conv9 = self.g_batch_norm["g_bn%i" % 12](conv9)
            conv9 = tf.nn.relu(conv9, name='conv9_relu1')
            conv9 = conv3d(conv9, self.gf_dim, k_h=3, k_w=3, k_d=3, d_h=1, d_w=1, d_d=1, name='conv9_conv2', w=gen_params["g_h%i_W" % 13], biases=gen_params["g_h%i_b" % 13])
            conv9 = self.g_batch_norm["g_bn%i" % 13](conv9)
            conv9 = tf.nn.relu(conv9, name='conv9_relu2')


            # output layer: (N, 640, 640, 32) -> (N, 640, 640, 1)
            logit = conv3d(conv9, self.channel + 1, k_h=1, k_w=1, k_d=1, d_h=1, d_w=1, d_d=1, name='conv_logit', w=gen_params["g_h%i_W" % 14], biases=gen_params["g_h%i_b" % 14])
